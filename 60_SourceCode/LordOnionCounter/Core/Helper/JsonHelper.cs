...
namespace LOC.Core.Helper
{
    static class JsonHelper
    {
        ...
        public static class JsonSerialization
        {
            ...
            public static T ReadFromJsonFile<T>(string filePath, Encoding Enc) where T : new()
            {
                ...
                try
                {
                    ...
                    return Newtonsoft.Json.JsonConvert.DeserializeObject<T>(fileContents.Replace("\\", "\\\\"));
                }
                finally
                {
                    if (reader != null)
            ...
            public static T ReadFromJsonFile<T>(string filePath, string EncName = "utf-16") where T : new()
            {
                return ReadFromJsonFile<T>(filePath, Encoding.GetEncoding(EncName));
            }

            public static object ReadFromJsonFile(string filePath, Encoding Enc)
            {
                TextReader reader = null;
                try
                {
                    reader = new StreamReader(filePath, Enc);
                    var fileContents = reader.ReadToEnd();
                    return Newtonsoft.Json.JsonConvert.DeserializeObject(fileContents.Replace("\\", "\\\\"));
                }
                finally
                {
                    if (reader != null)
